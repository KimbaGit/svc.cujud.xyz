    ...
    <div className="card-feedback bg-white rounded-2xl border border-slate-100 shadow-sm hover:shadow-md transition-shadow overflow-hidden"
      ...
      <div className="p-5">
        ...
        <div className="flex flex-wrap gap-3 mb-3">
          ...
          <span className="flex items-center gap-1 text-xs text-slate-400"><MessageSquare size={11} />{fb.nama}</span>
        ...
        {expanded && (
          <div className="mt-3 animate-fade-up space-y-2">
            <div className="rounded-xl bg-slate-50 border border-slate-100 p-3">
              <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-1">NIM</p>
              <p className="text-sm text-slate-700">{fb.nim}</p>
            </div>
            <div className="rounded-xl bg-slate-50 border border-slate-100 p-3">
            ...
            {fb.balasan && (
              <div className="rounded-xl p-3"
                style={{ backgroundColor: fb.status === "diterima" ? "#d1fae5" : "#fee2e2", borderLeft: `3px solid ${fb.status === "diterima" ? "#10b981" : "#ef4444"}` }}>
...
function FeedbackForm({ onSubmit }: { onSubmit: (data: Omit<Feedback, "id" | "status" | "createdAt">) => Promise<void> }) {
  const [form, setForm] = useState({ kategori: "", nama: "", nim: "", judul: "", deskripsi: "" });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await onSubmit(form);
      setSubmitted(true);
      setTimeout(() => { setSubmitted(false); setForm({ kategori: "", nama: "", nim: "", judul: "", deskripsi: "" }); setErrors({}); }, 2500);
        ...
        {[{ key: "nama", label: "Nama Lengkap", ph: "Nama Anda" }, { key: "nim", label: "NIM", ph: "Sesuai Angkatan" }].map(({ key, label, ph }) => (
          <div key={key}>
            ...
            <input type="text" value={form[key as keyof typeof form]} onChange={(e) => set(key, e.target.value)} placeholder={ph}
              ...
              style={{ borderColor: errors[key] ? "#ef4444" : "#e2e8f0", backgroundColor: "white" }}
              onFocus={(e) => (e.target.style.borderColor = "#0d9488")}
              onBlur={(e) => (e.target.style.borderColor = errors[key] ? "#ef4444" : "#e2e8f0")} />
            {errors[key] && <p className="text-xs text-red-500 mt-1 flex items-center gap-1"><AlertCircle size={11} />{errors[key]}</p>}
          </div>))}
      </div>

      <div>
        <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1.5">Judul Aduan *</label>
        <input type="text" value={form.judul} onChange={(e) => set("judul", e.target.value)} placeholder="Ringkasan singkat masalah Anda"
          className="w-full px-3 py-2.5 rounded-xl border text-sm transition-all outline-none"
...
export default function FeedbackPage() {
  ...
  const fetchFeedbacks = useCallback(async () => {
    ...
    } catch { setError("Tidak dapat terhubung ke database. Pastikan DATABASE_URL sudah dikonfigurasi."); }
    finally { setLoading(false); }
  }, []);

  useEffect(() => { fetchFeedbacks(); }, [fetchFeedbacks]);

  const handleNewFeedback = async (data: Omit<Feedback, "id" | "status" | "createdAt">) => {
  ...
  return (
    <div className="min-h-screen" style={{ backgroundColor: "#f8f7f4" }}>
      <header className="relative overflow-hidden noise-bg" style={{ backgroundColor: "#0f1b2d" }}>
        <div className="absolute inset-0 opacity-10" style={{ background: "radial-gradient(ellipse 80% 50% at 20% 40%, #0d9488, transparent), radial-gradient(ellipse 60% 40% at 80% 20%, #f59e0b, transparent)" }} />
        <div className="relative max-w-6xl mx-auto px-6 py-8">
          <div className="flex items-center justify-between mb-6">
            ...
            <a href="/admin" className="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium transition-all hover:opacity-80"
              style={{ backgroundColor: "rgba(255,255,255,0.1)", color: "#94a3b8", border: "1px solid rgba(255,255,255,0.1)" }}>
          ...
          <div className="flex flex-wrap gap-3 mt-6">
            {[{ label: "Total Aduan", value: stats.total, color: "#94a3b8" }, { label: "Menunggu", value: stats.menunggu, color: "#f59e0b" },
              { label: "Diterima", value: stats.diterima, color: "#10b981" }, { label: "Ditolak", value: stats.ditolak, color: "#ef4444" }].map((s) => (
              <div key={s.label} className="flex items-center gap-2 px-4 py-2 rounded-full"
                style={{ backgroundColor: "rgba(255,255,255,0.08)", border: "1px solid rgba(255,255,255,0.1)" }}>
                <span className="w-2 h-2 rounded-full" style={{ backgroundColor: s.color }} />
                <span className="text-xs text-slate-300">{s.label}: <strong className="text-white">{s.value}</strong></span>
              </div>
            ))}
          </div>
        </div>
          ...
          <section className="lg:col-span-3">
            <div className="flex items-center gap-3 mb-5">
              <div className="flex-1 flex items-center gap-2 overflow-x-auto pb-1">
                {(["semua", "menunggu", "diterima", "ditolak"] as const).map((s) => (
                  <button key={s} onClick={() => setFilterStatus(s)}
                    ...
                    style={filterStatus === s ? { backgroundColor: "#0f1b2d", color: "white" } : { backgroundColor: "white", color: "#64748b", border: "1px solid #e2e8f0" }}>
              ...
              <div className="flex items-center gap-2">
                <button onClick={fetchFeedbacks} title="Refresh"
                  className="p-2 rounded-full border border-slate-200 bg-white text-slate-400 hover:text-slate-600 transition-colors">
            ...
            {error && (
              <div className="rounded-2xl border border-red-100 bg-red-50 p-5 mb-4">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="text-sm font-semibold text-red-700 mb-1">Koneksi Database Gagal</p>
      ...
      <footer className="border-t border-slate-200 mt-12">
        <div className="max-w-6xl mx-auto px-6 py-6 flex flex-col md:flex-row items-center justify-between gap-3">
          <p className="text-xs text-slate-400">© 2026 SVC — Student Voice Campus Backup by <a href="https://etherthink.cujud.xyz/" className="text-emerald-600">Etherthink</a></p>
